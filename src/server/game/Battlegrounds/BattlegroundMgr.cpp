...
BattlegroundQueueTypeId BattlegroundMgr::BGQueueTypeId(BattlegroundTypeId bgTypeId, uint8 arenaType)
{
    switch (bgTypeId)
    {
        ...
        case BATTLEGROUND_RV:
            ...
            {
                case ARENA_TYPE_2v2:
        if (sWorld->getBoolConfig(CONFIG_ARENA_2v2_TEAM_ENABLE))
                    return BATTLEGROUND_QUEUE_2v2;
        else
                    return BATTLEGROUND_QUEUE_NONE;
                case ARENA_TYPE_3v3:
        if (sWorld->getBoolConfig(CONFIG_ARENA_3v3_TEAM_ENABLE))
                    return BATTLEGROUND_QUEUE_3v3;
        else
                    return BATTLEGROUND_QUEUE_NONE;
                case ARENA_TYPE_5v5:
        if (sWorld->getBoolConfig(CONFIG_ARENA_5v5_TEAM_ENABLE))
